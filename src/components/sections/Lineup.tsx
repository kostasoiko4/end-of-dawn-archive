...
const Lineup = () => {
  ...
  const { lineup: members, loading } = useSelector((state: RootState) => state.content);
  const [selectedMember, setSelectedMember] = useState<typeof members[0] | null>(null);

  const getImage = (member: typeof members[0]) => {
    if (member.image.startsWith('http') || member.image.startsWith('/') || member.image.startsWith('data:')) return member.image;
  ...
  return (
    <section id="lineup" className="py-24 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <h2 className="gothic-title text-3xl md:text-4xl text-center mb-4">{t('lineup.title')}</h2>
        <div className="section-divider mb-12" />

        <div className="flex flex-wrap justify-center gap-6 md:gap-8 max-w-7xl mx-auto">
          {loading ? (
            [...Array(7)].map((_, i) => (
              <div key={i} className="w-40 sm:w-48 md:w-56 lg:w-64">
                <Skeleton className="aspect-[3/4] rounded-lg bg-silver/10" />
                <Skeleton className="h-4 w-3/4 mt-3 bg-silver/10" />
                <Skeleton className="h-3 w-1/2 mt-1 bg-silver/10" />
              </div>
            ))
          ) : (
              ...
              >
                <div className="relative aspect-[3/4] rounded-lg overflow-hidden silver-border bg-charcoal-light transform transition-all duration-500 group-hover:scale-105 group-hover:shadow-[0_0_30px_rgba(139,92,246,0.3)]">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4 transform translate-y-0 group-hover:-translate-y-1 transition-transform duration-300">
                    <h3 className="font-cinzel text-silver text-base md:text-lg font-semibold drop-shadow-lg">{member.name}</h3>
                    <p className="text-primary text-sm md:text-base font-cinzel tracking-wider drop-shadow-lg">{member.role}</p>
                    <p className="text-muted-foreground text-xs md:text-sm mt-1 font-cormorant opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">{member.instrument}</p>
                  </div>
                </div>
              </div>
            ))
          ...
          >
            ...
            <div className="flex flex-col md:flex-row">
              <div className="relative md:w-1/2 aspect-[3/4] md:aspect-auto">
                <img src={getImage(selectedMember)} alt={selectedMember.name} className="w-full h-full object-cover" />
              ...
              <div className="p-6 md:p-8 md:w-1/2 flex flex-col justify-center">
                <h3 className="gothic-title text-2xl md:text-3xl mb-2">{selectedMember.name}</h3>
                <p className="text-primary font-cinzel text-lg tracking-widest mb-4">{selectedMember.role}</p>
                <div className="section-divider mb-4" />
                <p className="text-muted-foreground font-cormorant text-base mb-2">{selectedMember.instrument}</p>
                <p className="text-silver/80 font-cormorant text-lg italic">"{selectedMember.bio}"</p>
              </div>
            </div>
          </div>
        </div>
